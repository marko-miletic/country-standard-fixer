...
def test_get_country_columns1():
    dataframe = pd.read_csv(os.path.join(INPUT_FILES_DIRECTORY, 'test1.csv'))
    assert get_country_columns(dataframe, standardized_values.get_countries()) == ['name']


def test_get_country_columns2():
    dataframe = pd.read_json(os.path.join(INPUT_FILES_DIRECTORY, 'test3.json'))
...
def test_correction():
    ...
    standard_file = pd.read_csv(os.path.join(STANDARD_VALUES_DIRECTORY, 'iso_countries.csv'))
    input_file = pd.read_csv(os.path.join(INPUT_FILES_DIRECTORY, 'test1.csv'))
    ...
    assert (standard_countries == input_countries
            and standard_codes_alpha3 == input_codes_alpha3
            and standard_codes_alpha2 == input_codes_alpha2)
